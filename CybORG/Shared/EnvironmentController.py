...
class EnvironmentController(CybORGLogger):
    ...
    def __init__(self, scenario_generator: ScenarioGenerator, agents, np_random, time_limit):
    ...
    def _get_reward(self, reset=False):
        for agent_name, team_calcs in self.team_reward_calculators.items():
            self.reward[agent_name] = {}
            for calc_name, r_calc in team_calcs.items():
                if reset:
                    r_calc.reset()
    ...
    def step(self, actions: dict = None, skip_valid_action_check=False):
        ...
        self.action = actions
        self.state.update_actions(actions)
        ...
        # Reset all blocked subnets
        self.state.reset_blocks()
    ...
    #CyMARL - assumes agents belong to only one team
    def get_reward(self, agent_name):
        #team = [team_name for agent_name, team_name in self.team_assignments.items()][0]
        return self.reward[agent_name]
