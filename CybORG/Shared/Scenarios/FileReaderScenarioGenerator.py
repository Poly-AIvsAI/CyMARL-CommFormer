...
class FileReaderScenarioGenerator(ScenarioGenerator):
    ...
    def __init__(self, file_path: str):
        ...
        self.update_each_step = True # CyMARL added for Cyborg 3 compatibility

    def create_scenario(self, np_random) -> Scenario:
        with open(self.file_path) as fIn:
            scenario_dict = yaml.load(fIn, Loader=yaml.FullLoader)
        # load images
        images_file_path = os.path.dirname(self.file_path) + "/images/"
        ...
        # parse team name and calculator
        ...
        scenario_dict['team_agents'] = {agent_name: [(agent_data['Team'])] for agent_name, agent_data in scenario_dict['Agents'].items()}
        # add internal only flag
        scenario_dict['internal_only'] = {agent_name: (agent_data['internal_only']) for agent_name, agent_data in scenario_dict['Agents'].items()}
        ...
        #    with open(f'{cyborg_path[:-10]}/render/example_data_old_scenario.json', 'r') as f:
        #        data = json.load(f)
