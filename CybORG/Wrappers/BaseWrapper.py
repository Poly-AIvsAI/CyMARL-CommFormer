...
class BaseWrapper:
    ...
    def multi_step(self, actions: dict):
        ...
        for agent in self.agents:
            results[agent].observation = self.observation_change(results[agent].observation, agent=agent)
            results[agent].action_space = self.action_space_change(results[agent].action_space)
        return results

    def reset(self, agent=None, seed = None):
        result = self.env.reset(agent, seed)
        result.observation = self.observation_change(result.observation, agent=agent)
        result.action_space = self.action_space_change(result.action_space)
        return result

    def observation_change(self, observation, agent: str = None):
        return observation
    ...
    def get_observation(self, agent: str):
        return self.observation_change(self.env.get_observation(agent), agent=agent)
