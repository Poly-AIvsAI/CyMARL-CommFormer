    ...
    def get_avail_agent_actions(self, comm_vector, step, agent_id):
        ...
        for i, action in enumerate(actions):
            if action.name == 'Sleep' or action.name == 'Block': # Sleep action and block action is always available
                ...
                host_obs = obs[(i - 1) % (len(obs) - 1)] # issue may arise if the first action is not Sleep
                if sum(host_obs) > 1: # If he host has more than 1 bit then all actions are available
                    valid_actions[i] = 1
